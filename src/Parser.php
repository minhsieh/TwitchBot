...
class Parser
{
    ...
    public function __construct()
    {
        ...
        $middle = "(?:[^:]+)";
        $trailing = "(?:[^$xdelim]+)";
        $this->typeReg = "/^$xdelim(?P<type>[^ ]+)(?P<params> [^$xdelim]+)?$xdelim$/S";
    ...
    public function parse($message)
    {
        ...
        if (isset($this->params[$type]) && !empty($parsed['params'])) {
            preg_match($this->params[$type], $parsed['params'], $params);
            ...
            switch ($type) {
                ...
                case 'PRIVMSG':
                    if ($params && preg_match($this->commandReg, $params['text'] , $parsed_command)){
                ...
                case 'NOTICE':
                    if ($params && preg_match($this->typeReg, end($params), $type)) {
                        $parsed['type'] = $this->removeIntegerKeys($type);
                        if (isset($this->typeParams[$parsed['type']['command']])
                            && !empty($parsed['type']['params'])) {
                            $typeParams = ltrim($parsed['type']['params']);
                            preg_match($this->typeParams[$parsed['type']['command']], $typeParams, $typeParsed);
                            $parsed['type']['params'] = array_merge(
                                $this->removeIntegerKeys($typeParsed),
                                array('all' => $typeParams)
                            );
                        }
                    }
                    break;
            ...
            if (isset($this->replies[$type])) {
                $parsed['code'] = $this->replies[$type];
            } else {
                $parsed['code'] = $type;
            }
